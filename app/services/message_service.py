...
from app.externals.s3_upload.s3_upload_client import check_file_exists_direct, upload_file
...
class MessageService(MessageServiceInterface):
    ...
    async def handle_message(self, request: MessageRequest):
        ...
        return await self.conversation_manager.process_conversation(request=request, agent_config=agent_config)
    ...
    async def handle_message_with_config(self, request: MessageRequest):
        ...
        return {"message": message_response, "agent_config": agent_config}
    ...
    async def process_multiple_agents(self, agent_queries: list[dict]) -> dict:
        tasks = [
            self.handle_message(MessageRequest(agent_id=item["agent"], conversation_id="", query=item["query"]))
    ...
    async def generate_pdf(self, request: GeneratePdfRequest):
        ...
        agent_queries = [
            {"agent": "agent_copies_pdf", "query": f"section: {section}. {base_query} "} for section in sections.keys()
        ]
        ...
        result = await upload_file(S3UploadRequest(file=pdf, folder=folder_path, filename=base_filename))

        return result

    async def resolve_funnel(self, request: ResolveFunnelRequest):
        pain_detection_response = await self.handle_message(
            MessageRequest(
        ...
        buyer_detection_response = await self.handle_message(
            MessageRequest(
        ...
        sales_angles_response = await self.handle_message_json(
            MessageRequest(
    ...
    async def resolve_brand_context(self, request: BrandContextResolverRequest):
        brand_agent_task = self.handle_message_json(
            MessageRequest(
        ...
        context_agent_task = self.handle_message_json(
            MessageRequest(
        ...
        return {"brands": brands, "contexts": contexts}
